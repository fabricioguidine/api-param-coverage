...
def step_see_error_message(context, message):
    ...
    assert message.lower() in context.validation_error.lower(), \
        f"Expected '{message}' in error: {context.validation_error}"
...
def step_processing_endpoint(context, current, total):
    ...
    context.progress_bar.update(current, status=f"Current: /api/users")
...
def step_llm_processing_chunks(context, count):
    ...
    context.progress_bars = [
        ProgressBar(total=12, description=f"Chunk {i+1}/{count}")
...
def step_select_alternative_location(context):
    ...
    assert hasattr(context, 'recovery_options')
    assert "Change output directory" in str(context.recovery_options) or \
           "Change" in str(context.recovery_options)
