...
from ..brd import BRDGenerator, BRDLoader, BRDParser, BRDSchema
from ..cli import ErrorHandler, InteractiveSelector, print_error, print_info, print_success, print_warning
from ..utils.constants import DEFAULT_COVERAGE_PERCENTAGE, MAX_COVERAGE_PERCENTAGE, MIN_COVERAGE_PERCENTAGE


def handle_brd_selection(brd_loader: BRDLoader, api_key: str, status_updater) -> Tuple[Optional[BRDSchema], Optional[str]]:
    """
    ...
    selected_brd = InteractiveSelector.select_from_list(available_brds, prompt="Select BRD schema file", allow_cancel=True)
...
def handle_brd_parsing(api_key: str, input_dir: Path) -> Optional[BRDSchema]:
    """
    ...
    documents = [
        f for f in input_dir.iterdir() if f.is_file() and f.suffix.lower() in [".pdf", ".doc", ".docx", ".txt", ".csv", ".md"]
    ]
...
def handle_brd_generation(
    ...
    brd_generator = BRDGenerator(api_key=api_key, model=model, analytics_dir=analytics_dir)
