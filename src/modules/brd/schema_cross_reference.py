...
class SchemaCrossReference:
    ...
    def filter_endpoints_by_brd(self, analysis_data: Dict[str, Any], brd: BRDSchema) -> Dict[str, Any]:
        """
    ...
    def get_brd_coverage_report(self, analysis_data: Dict[str, Any], brd: BRDSchema) -> Dict[str, Any]:
        """
        ...
        for endpoint in all_endpoints:
            ...
            endpoint_info = {"path": path, "method": method, "parameters_count": len(endpoint.get("parameters", []))}

            if (path, method) in brd_endpoint_set:
                requirements = brd.get_requirements_for_endpoint(path, method)
                endpoint_info["requirements"] = [
                    {"requirement_id": req.requirement_id, "title": req.title, "priority": req.priority.value}
                    for req in requirements
                ]
