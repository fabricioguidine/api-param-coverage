...
from .brd_schema import BRDSchema
...
class BRDParser:
    ...
    def __init__(
        ...
        self.llm_prompter = (
            LLMPrompter(model=model, api_key=api_key, provider=provider, run_output_dir=None) if api_key else None
        )
    ...
    def parse_document(self, filename: str) -> Optional[BRDSchema]:
        ...
        if not file_path_obj.exists():
            ...
            print("   Tip: Ensure the file exists or provide the full path.")
            return None
        ...
        if file_ext not in self.SUPPORTED_FORMATS:
            ...
            print("   Tip: Convert your document to one of the supported formats.")
            return None
        ...
        brd = transformer.transform_to_schema(
            source_data={"content": content, "filename": file_path_obj.name}, source_type="document"
        )
