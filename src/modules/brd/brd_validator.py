...
class BRDValidator:
    ...
    def validate_brd_against_swagger(self, brd: BRDSchema, analysis_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        swagger_endpoint_set = {(ep.get("path", ""), ep.get("method", "").upper()) for ep in swagger_endpoints}
    ...
    def _fuzzy_match_endpoint(self, path: str, method: str, swagger_endpoint_set: set) -> Optional[Tuple[str, str]]:
        """
    ...
    def _suggest_similar_endpoint(self, path: str, method: str, swagger_endpoint_set: set) -> Optional[str]:
        """
        ...
        same_method_endpoints = [(p, m) for p, m in swagger_endpoint_set if m == method.upper()]
        ...
        for swagger_path, _ in same_method_endpoints:
            ...
            if len(brd_segments) == len(swagger_segments):
                matches = sum(
                    1 for b, s in zip(brd_segments, swagger_segments) if b == s or b.startswith("{") or s.startswith("{")
                )
    ...
    def generate_validation_report(self, validation_report: Dict[str, Any], output_path: Optional[Path] = None) -> Path:
        """
