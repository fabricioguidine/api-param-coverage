...
def detect_provider_from_key(api_key: str) -> str:
    ...
    if (api_key.startswith("AIza") or len(api_key) > 100) and re.match(r"^[A-Za-z0-9_-]+$", api_key) and len(api_key) > 30:
...
def get_provider_info(provider: str) -> dict:
    ...
    return providers.get(
        provider, {"name": "Unknown", "models": ["gpt-4"], "default_model": "gpt-4", "env_var": "LLM_API_KEY"}
    )
...
def setup_api_key() -> Tuple[Optional[str], Optional[str]]:
    ...
    while True:
        ...
        try:
            ...
            print("  This will not be asked again.")
