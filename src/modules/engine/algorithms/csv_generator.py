...
class CSVGenerator:
        ...
        for line in lines:
            ...
            if line.startswith("Scenario:") or line.startswith("Scenario Outline:"):
                ...
                if current_scenario and current_steps:
                    rows.append(self._create_csv_row(current_feature, current_scenario, current_steps, current_tags))
            ...
            if in_examples and not line.startswith("|"):
                ...
                for example_row in example_rows:
                    ...
                    rows.append(self._create_csv_row(current_feature, scenario_with_example, steps_with_example, current_tags))
                example_rows = []
                continue
    ...
    def _create_csv_row(
        self, feature: Optional[str], scenario: Optional[str], steps: List[str], tags: List[str]
    ) -> Dict[str, Any]:
    ...
    def save_to_csv(self, data: List[Dict[str, Any]], filename: str, fieldnames: Optional[List[str]] = None) -> str:
        """
        ...
        timestamp = self.run_timestamp or datetime.now().strftime("%Y%m%d_%H%M%S")
    ...
    def gherkin_to_csv(self, gherkin_content: str, swagger_name: str) -> str:
        """
