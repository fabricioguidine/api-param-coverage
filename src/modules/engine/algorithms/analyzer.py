...
class SchemaAnalyzer:
    ...
    def analyze_schema_file(self, schema_filename: str) -> Dict[str, Any]:
        ...
        with open(schema_path, encoding="utf-8") as f:
            schema = yaml.safe_load(f) if schema_path.suffix in [".yaml", ".yml"] else json.load(f)
    ...
    def analyze_schema(self, schema: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for path, path_item in paths.items():
            ...
            for method, operation in path_item.items():
                ...
                if method.lower() in ["get", "post", "put", "delete", "patch", "head", "options", "trace"] and isinstance(
                    operation, dict
                ):
                    endpoint_data = self._analyze_endpoint(path, method.upper(), operation, common_params, schema)
    ...
    def _analyze_endpoint(
        self, path: str, method: str, operation: Dict[str, Any], common_params: List[Dict[str, Any]], schema: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        for param in all_params:
            ...
            if param_def.get("in") == "body":
                ...
                body_params = self._extract_schema_properties(body_schema, "body", schema, prefix="")
                parameters.extend(body_params)
        ...
        return {"path": path, "method": method, "parameters": parameters}

    def _find_parameter(self, params: List[Dict[str, Any]], name: str, location: str) -> Optional[Dict[str, Any]]:
    ...
    def _analyze_parameter(self, param: Dict[str, Any], location: str, schema: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _analyze_request_body(self, request_body: Dict[str, Any], schema: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    def _analyze_responses(self, responses: Dict[str, Any], schema: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    def _extract_schema_properties(
        self, schema: Dict[str, Any], location: str, root_schema: Dict[str, Any], prefix: str = "", depth: int = 0
    ) -> List[Dict[str, Any]]:
        ...
        if schema.get("type") == "object" or "properties" in schema:
            ...
            for prop_name, prop_schema in properties.items():
                ...
                if prop_schema.get("type") == "object" or "properties" in prop_schema:
                    nested = self._extract_schema_properties(prop_schema, location, root_schema, f"{full_name}.", depth + 1)
        ...
        # Handle array type
            ...
            if items_schema.get("type") == "object" or "properties" in items_schema:
                nested = self._extract_schema_properties(items_schema, location, root_schema, f"{prefix}[]", depth + 1)
                parameters.extend(nested)
    ...
    def _compute_iteration_count(self, schema: Dict[str, Any], constraints: Dict[str, Any]) -> Any:
    ...
    def _generate_notes(self, schema: Dict[str, Any], constraints: Dict[str, Any], location: str) -> str:
