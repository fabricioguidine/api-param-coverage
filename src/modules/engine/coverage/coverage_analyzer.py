...
from ...brd.brd_schema import BRDSchema
...
class CoverageAnalyzer:
    ...
    def __init__(
        self, analytics_dir: Optional[str] = None, run_output_dir: Optional[Path] = None, run_timestamp: Optional[str] = None
    ):
        """
    ...
    def analyze_coverage(self, gherkin_content: str, brd: BRDSchema, analysis_data: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    def _map_scenarios_to_requirements(self, scenarios: List[Dict[str, Any]], brd: BRDSchema) -> Dict[str, Dict[str, Any]]:
        """
        ...
        for scenario in scenarios:
            ...
            if endpoint_info:
                ...
                for requirement in brd.requirements:
                    if requirement.endpoint_path == path and requirement.endpoint_method.upper() == method.upper():
        ...
        for _req_id, coverage in requirement_coverage.items():
    ...
    def _calculate_coverage_metrics(self, requirement_coverage: Dict[str, Dict[str, Any]], brd: BRDSchema) -> Dict[str, Any]:
        """
        ...
        """
        total_requirements = len(brd.requirements)
        covered_requirements = sum(1 for cov in requirement_coverage.values() if cov["scenario_count"] > 0)
    ...
    def _identify_missing_scenarios(
        self, requirement_coverage: Dict[str, Dict[str, Any]], brd: BRDSchema
    ) -> List[Dict[str, Any]]:
        """
    ...
    def _identify_coverage_gaps(self, requirement_coverage: Dict[str, Dict[str, Any]], brd: BRDSchema) -> List[Dict[str, Any]]:
        """
        ...
        gaps.sort(
            key=lambda x: (0 if x["priority"] == "critical" else 1 if x["priority"] == "high" else 2, x["coverage_percentage"])
        )

        return gaps

    def generate_coverage_report(self, coverage_report: Dict[str, Any], output_path: Optional[Path] = None) -> Path:
        """
        ...
        lines.append("-" * 80)
        for _req_id, coverage in coverage_report["requirement_coverage"].items():
