...
class LLMPrompter:
    ...
    def create_prompt(
        self, processed_data: Dict[str, Any], task: str = "analyze", analysis_data: Optional[Dict[str, Any]] = None
    ) -> str:
        """
        ...
        if task == "gherkin" and analysis_data:
            ...
            estimated_tokens + template_tokens
...
""",
    ...
    def _format_endpoints(self, endpoints: List[Dict[str, Any]]) -> str:
        ...
        for endpoint in endpoints[:20]:  # Limit to first 20 for prompt size
            formatted.append(f"- {endpoint['method']} {endpoint['path']} ({endpoint.get('summary', 'No summary')})")
        ...
        try:
            ...
            if self.provider == "azure":
                ...
                if not endpoint:
                    ...
                    return None
                client = AzureOpenAI(api_key=self.api_key, azure_endpoint=endpoint, api_version=api_version)
            else:
            ...
            if total_estimated > 8000:  # GPT-4 limit is 8192
                print(
                    f"⚠ Warning: Large prompt detected (~{prompt_tokens_estimate} tokens input + {max_tokens_for_response} output = ~{total_estimated} total)"
                )
                print("   GPT-4 limit is 8192 tokens. Reducing chunk size or using compact summary...")
                # If we're over limit, we should have chunked or reduced more
                if total_estimated > 8192:
                    raise ValueError(
                        f"Prompt too large ({total_estimated} tokens) for GPT-4 (8192 limit). Use smaller chunks or different model."
                    )
    ...
    def _aggressively_reduce_analysis_data(self, analysis_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if total_endpoints > max_endpoints:
            ...
            endpoints_sorted = sorted(
                endpoints, key=lambda e: (method_priority.get(e.get("method", ""), 99), e.get("path", ""))
            )
        ...
        return {"endpoints": optimized_endpoints, "total": total_endpoints, "shown": len(optimized_endpoints)}
    ...
    def _optimize_analysis_data(self, analysis_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "endpoints" in optimized:
            ...
            for endpoint in endpoints:
                ...
                optimized_endpoint = {"path": endpoint.get("path", ""), "method": endpoint.get("method", ""), "parameters": []}
    ...
    def generate_gherkin_scenarios(
        self, processed_data: Dict[str, Any], analysis_data: Dict[str, Any], use_chunking: bool = True
    ) -> Optional[str]:
        """
    ...
    def _generate_gherkin_with_chunking(self, processed_data: Dict[str, Any], analysis_data: Dict[str, Any]) -> Optional[str]:
        ...
        for i in range(0, total_endpoints, chunk_size):
            ...
            print(
                f"   Processing chunk {chunk_num}/{total_chunks} (endpoints {i + 1}-{min(i + chunk_size, total_endpoints)})..."
            )
            ...
            try:
                chunk_scenarios = self.process_and_prompt(processed_data, task="gherkin", analysis_data=chunked_analysis)
